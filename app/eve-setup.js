...
(function(global){
  ...
  function normalise(current,{legacy=false}={}){
    ...
    if(global.EveDeployment?.isGoogleWorkspace?.()){
      next.storageProvider='google';
      next.relayMode='google-workspace';
      next.evaluationMode=false;
    }else if(global.EveDeployment?.isOrganisationCloud?.()){
      const cfg=global.EveDeployment.runtimeConfig?.()||{},preferred=String(cfg.defaultStorageProvider||'').toLowerCase();
      next.storageProvider=cfg.organisationStorage?.enabled!==false?'organisation':preferred==='google'?'google':preferred==='microsoft'?'microsoft':next.storageProvider;
      next.relayMode='organisation';
      next.evaluationMode=false;
    }
    ...
    next.relayMode=next.relayMode==='google-workspace'?'google-workspace':next.relayMode==='organisation'?'organisation':next.relayMode==='local'?'local':'cloudflare';
  ...
  function relayReady(){
    if(global.EveDeployment?.isOrganisationCloud?.())return true;
    if(global.EveDeployment?.isGoogleWorkspace?.())return !!state?.setup?.relayVerified;
  ...
  function chooseStorage(provider){
    const p=global.EveDeployment?.isGoogleWorkspace?.()?'google':provider==='organisation'?'organisation':provider==='google'?'google':'microsoft';
  ...
  async function connectStorage(provider){
    const p=global.EveDeployment?.isGoogleWorkspace?.()?'google':provider;
    chooseStorage(p);
    if(global.EveDeployment?.isGoogleWorkspace?.()){
      try{
        const ownerKey=String(state.setup?.relayOwnerKey||'').trim();
        if(!ownerKey)throw new Error('Return to your copied Google Sheet and choose Eve → Set up / open Eve.');
        const info=await global.EveDeployment.bootstrap(ownerKey);
        state.storage=state.storage||{};
        state.storage.provider='Google Drive';
        state.storage.connected=true;
        state.storage.location=info.location||'My Drive / Eve';
        state.storage.ownership=info.storageOwnership||state.storage.ownership||'My Drive';
        state.storage.connectors={google:null,microsoft:null,...(state.storage.connectors||{})};
        state.storage.connectors.google={capability:'apps-script',connection:{connected:true,location:{displayName:state.storage.location,rootFolderId:info.rootFolderId||''}}};
        if(info.researcherUrl)setSetup({researcherAppUrl:info.researcherUrl,participantAppUrl:info.participantUrl||info.researcherUrl,relayVerified:true,relayLastError:''},{rerender:false});
        await persistWorkspace(false,{skipCloud:true});
        render();
        toast('Google Drive connected',2600,'success');
      }catch(err){toast(`Google Drive setup failed: ${err.message}`,5500,'error')}
      return;
    }
    global.EveCloud?.connect?.(p);
  }
  ...
  async function refreshStorage(){
    try{
      ...
      }else if(global.EveDeployment?.isOrganisationCloud?.()&&state.setup?.storageProvider==='organisation'){
        await global.EveCloud?.initialise?.();await global.EveCloud?.setDefaultProvider?.('organisation');
      }else{
  ...
  async function testRelay(){
    ...
    if(global.EveDeployment?.isOrganisationCloud?.()){
      const ok=await relayHealth().catch(()=>false);
      setSetup({relayVerified:ok,relayCheckedAt:Date.now(),relayLastError:ok?'':'The organisation Eve service is not reachable.'});
      return ok;
    }
    if(global.EveDeployment?.isGoogleWorkspace?.()){
      if(!setup.relayOwnerKey){
        setSetup({relayVerified:false,relayLastError:'Return to your copied Google Sheet and choose Eve → Set up / open Eve.'});
        return false;
      }
      try{
        const health=await global.EveDeployment.relayFetch('/api/health',{cache:'no-store'},8000);
        if(!health.ok)throw new Error(`Participant connection returned HTTP ${health.status}`);
        const owner=await global.EveDeployment.relayFetch('/api/owner-check',{cache:'no-store',headers:ownerHeaders(state)},8000);
        if(!owner.ok)throw new Error('The researcher owner key does not match this Apps Script project.');
  ...
  async function runChecks(){
    ...
    checks.relay.detail=checks.relay.ok?(global.EveDeployment?.isGoogleWorkspace?.()?'Google Apps Script participant connection ready':state.setup.relayMode==='local'?'Local evaluation relay reachable':state.setup.relayUrl):state.setup.relayLastError||'Participant connection is not reachable';
    try{
  ...
  function progress(step){
    const labels=(global.EveDeployment?.isGoogleWorkspace?.()||global.EveDeployment?.isOrganisationCloud?.())?['Welcome','Storage','Participant','Recovery','Ready']:['Welcome','Storage','Relay','Recovery','Ready'];
  ...
  function readyStep(){
    return `<div class="setup-step"><div class="setup-kicker">4 · FINAL CHECK</div><h1>Check Eve is ready to run research.</h1><p class="setup-lead">This checks organisation storage, the participant connection, browser encryption and recovery together.</p>
      ${checksMarkup()}
  ...
  function view(){
    ...
    const providerNative=global.EveDeployment?.isGoogleWorkspace?.()||global.EveDeployment?.isOrganisationCloud?.(),nativeBody=providerNative?global.EveDeployment.providerSetupStep?.(step):null;
    const body=nativeBody||(step===0?welcomeStep():step===1?storageStep():step===2?relayStep():step===3?recoveryStep():readyStep());
