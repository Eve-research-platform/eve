...
(function(global){
  ...
  function providerKey(value){
    ...
    if(v==='organisation'||v==='organization'||v==='organisation cloud'||v==='organisation cloud storage'||v==='eve cloud storage')return'organisation';
  ...
  function providerLabel(key){return key==='organisation'?(global.EveDeployment?.runtimeConfig?.().organisationStorage?.label||'Organisation cloud storage'):key==='google'?'Google Drive':'SharePoint'}
  ...
  function connectorReady(key){
    const p=providerKey(key);
    if(p==='organisation')return !!(global.EveDeployment?.isOrganisationCloud?.()&&global.EveDeployment?.runtimeConfig?.().organisationStorage?.enabled!==false);
    if(p==='google'&&global.EveDeployment?.isGoogleWorkspace?.())return !!state?.storage?.connected;
    const c=connector(key);
  ...
  function locationLabel(key){
    ...
    return p==='organisation'?(global.EveDeployment?.runtimeConfig?.().organisationStorage?.label||'Organisation cloud storage'):c?.connection?.location?.displayName||(p==='google'?'My Drive / Eve':'SharePoint / Eve')
  }
  ...
  function refreshDerivedStorage(){
    const p=providerKey(state.storage?.provider)||(global.EveDeployment?.isOrganisationCloud?.()?'organisation':'microsoft');
    state.storage.provider=p==='organisation'?providerLabel('organisation'):p==='google'?'Google Drive':'SharePoint';
    state.storage.connected=connectorReady(p);
    state.storage.location=connectorReady(p)?locationLabel(p):(p==='organisation'?providerLabel('organisation'):p==='google'?'My Drive / Eve':'Customer Research / Eve');
  }
  function resolvedStudyStorage(study){
    ...
    return{
      provider:p==='organisation'?providerLabel('organisation'):p==='google'?'Google Drive':'SharePoint',
  ...
  async function initialise(){
    ensureStorageShape();setupBridge();
    if(global.EveDeployment?.isGoogleWorkspace?.()){
      state.storage.provider='Google Drive';
      if(state.setup?.relayOwnerKey){
        try{
          const info=await global.EveDeployment.storageInfo(state.setup.relayOwnerKey);
          state.storage.connected=!!info?.connected;
          if(info?.location)state.storage.location=info.location;
          if(info?.storageOwnership)state.storage.ownership=info.storageOwnership;
          if(state.storage.connected)state.storage.connectors.google={capability:'apps-script',connection:{connected:true,location:{displayName:state.storage.location,rootFolderId:info.rootFolderId||''}}};
        }catch{}
      }
      refreshDerivedStorage();if(state.view==='storage')render();return;
    }
    if(global.EveDeployment?.isOrganisationCloud?.()&&global.EveDeployment?.runtimeConfig?.().organisationStorage?.enabled!==false){
      state.storage.provider=providerLabel('organisation');state.storage.connected=true;state.storage.location=providerLabel('organisation');
  ...
  async function setDefaultProvider(value){
    const p=providerKey(value)||(global.EveDeployment?.isOrganisationCloud?.()?'organisation':'microsoft');
    state.storage.provider=p==='organisation'?providerLabel('organisation'):p==='google'?'Google Drive':'SharePoint';refreshDerivedStorage();
  ...
  async function cloudWrite(key,path,content){
    const p=providerKey(key);
    if(p==='organisation')return api('/api/organisation-storage/files',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({path,content:String(content)})},120000);
    if(p==='google'&&global.EveDeployment?.isGoogleWorkspace?.())return global.EveDeployment.storageWrite(state.setup?.relayOwnerKey||'',path,String(content));
  ...
  async function cloudRead(key,path,{optional=false}={}){
    const p=providerKey(key);
    if(p==='organisation'){
      try{return await api(`/api/organisation-storage/files?path=${encodeURIComponent(path)}`,{},120000)}
      catch(err){if(optional&&err.status===404)return null;throw err}
    }
    if(p==='google'&&global.EveDeployment?.isGoogleWorkspace?.()){
      try{return await global.EveDeployment.storageRead(state.setup?.relayOwnerKey||'',path)}
      catch(err){if(optional&&/not found/i.test(err.message))return null;throw err}
    }
    const capability=cap(key);
  ...
  async function cloudDelete(key,path){
    const p=providerKey(key);
    if(p==='organisation')return api('/api/organisation-storage/files',{method:'DELETE',headers:{'Content-Type':'application/json'},body:JSON.stringify({path})},120000);
    if(p==='google'&&global.EveDeployment?.isGoogleWorkspace?.())return global.EveDeployment.storageDelete(state.setup?.relayOwnerKey||'',path);
    const capability=cap(key);
  ...
  async function cloudList(key,prefix=''){
    const p=providerKey(key);
    if(p==='organisation')return api(`/api/organisation-storage/files/list?prefix=${encodeURIComponent(prefix)}`,{},120000);
    if(p==='google'&&global.EveDeployment?.isGoogleWorkspace?.())return global.EveDeployment.storageList(state.setup?.relayOwnerKey||'',prefix);
    const capability=cap(key);
  ...
  async function recordingDocFor(study,response,id,meta){
    ...
    if(meta?.storage==='relay'&&study.relayPublished&&study.relayAdminToken){
      try{
        const r=await global.EveDeployment.relayFetch(`/api/studies/${encodeURIComponent(study.slug)}/recordings/${encodeURIComponent(id)}`,{cache:'no-store',headers:EveSetup.ownerHeaders(state,{'X-ResearchOS-Admin':study.relayAdminToken||''})},120000);
  ...
  async function retireRelayRecording(p,path,doc,study,response,id,meta){
    if(meta?.storage!=='relay'||state.setup?.relayMode!=='cloudflare')return false;
    try{
      const verified=await cloudRead(p,path),check=JSON.parse(verified?.content||'{}');
      if(check.format!=='eve-cloud-recording'||String(check.recordingId)!==String(id)||String(check.responseId)!==String(response.id))throw new Error('Durable recording verification failed');
      const raw=relayVersionKey(study,response.studyVersion,false)||study.relayKey;if(!raw)throw new Error('Study recording key is unavailable');
      const blob=await decryptRecordingBlobWithKey(doc.envelope,raw);await saveLocalRecording(id,blob);
      const rr=await global.EveDeployment.relayFetch(`/api/studies/${encodeURIComponent(study.slug)}/recordings/${encodeURIComponent(id)}`,{method:'POST',headers:EveSetup.ownerHeaders(state,{'X-ResearchOS-Admin':study.relayAdminToken||''})},20000);
      if(!rr.ok)throw new Error(`Relay retention returned HTTP ${rr.status}`);const out=await rr.json();
      meta.storage='local';meta.durableProvider=p;meta.durablePath=path;meta.durableVerifiedAt=Date.now();meta.relayPurgeAfter=Number(out?.retention?.purgeAfter)||null;
      await idbSet(RESPONSE_PREFIX+response.id,JSON.stringify(await encrypt(response)));return true
    }catch(err){console.warn('Relay recording retained because durable handoff was not fully verified',id,err);return false}
  }
    ...
    const task=(async()=>{
      ...
      try{
        ...
        for(const r of responses){
          const path=`Studies/${r.studyId}/responses/${r.id}.eve.json`,s=studies.find(x=>x.id===r.studyId),refs=recordingRefs(r.answers||{});responsePaths.push(path);
          if(s)for(const [id,meta] of refs){const doc=await recordingDocFor(s,r,id,meta);if(!doc)continue;const rp=`Studies/${r.studyId}/recordings/${id}.eve.json`;recordingPaths.push(rp);await cloudWrite(p,rp,JSON.stringify(doc));await retireRelayRecording(p,rp,doc,s,r,id,meta)}
  ...
  function storagePage(){
    ensureStorageShape();
    if(global.EveDeployment?.isGoogleWorkspace?.())return global.EveDeployment.googleStoragePage();
    if(!connectorConfig.loaded&&!connectorConfig.loading)setTimeout(()=>loadConfig(),0);
